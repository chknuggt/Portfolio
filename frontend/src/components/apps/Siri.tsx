...
//  Marios resume context for conversational answers
const MARIOS_INFO = `Marios Eleftheriou is a Junior Full-Stack & Mobile Developer working at IANUS Technologies in Cyprus.
GitHub: https://github.com/chknuggt
He builds full-stack projects using React, Next.js, TypeScript, Django, Flutter, Supabase, and PostgreSQL.
His resume is available for download.`;

const SYSTEM_PROMPT = `You are Siri, Marios's virtual personal voice assistant running inside a macOS-like web portfolio.
You control the interface through tool calls. Keep replies short and natural (1-2 sentences max).

IMPORTANT RULES:
1. If you don't understand the request, or it is unrelated, reply EXACTLY: "Hi, I am Siri, Marios's virtual personal voice assistant."
2. If asked about Marios, his skills, experience, resume, or background — ALWAYS call the download_resume tool AND explicitly say in your reply that Marios has provided you with his resume and you are downloading it for the user.
...
const TOOLS = [
  ...
  { type: "function", function: { name: "download_resume", description: "Downloads Marios's resume PDF. Call this whenever the user asks about Marios, his background, skills, experience, or wants to see his resume." } }
];
...
export default function Siri({ closeSiri }: { closeSiri?: () => void }) {
  ...
  const downloadResume = useCallback(() => {
    ...
    link.download = "Marios_Eleftheriou_CV.pdf";
    link.target = "_blank";
  ...
  const executeTool = useCallback(async (name: string, args: any): Promise<string> => {
    ...
    switch (name) {
      ...
      case "download_resume":
        downloadResume();
        return "Here's Marios's resume! The download should start now.";
  ...
  const executeAgent = useCallback(async (userText: string) => {
    try {
      ...
      if (!reply || !reply.trim()) {
        reply = "Hi, I am Siri, Marios's virtual personal voice assistant.";
      }
      ...
      const fallback = "Hi, I am Siri, Marios's virtual personal voice assistant.";
